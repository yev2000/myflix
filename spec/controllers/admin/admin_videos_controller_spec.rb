...
describe Admin::VideosController do 
  ...
  describe "POST update" do
    before do
      ...
      @action = Fabricate(:category)
      @drama = Fabricate(:category)
      @video = Fabricate(:video, title: "Caddyshack", description: "A funny movie with Bill Murray", categories: [@action, @drama])
    end

    context "no logged in user" do
      it_behaves_like("require_sign_in") { let(:action) { post :update, id: @video.id, video: {title: "New Title", category_ids: [@comedies.id.to_s], description: "A new movie instead" } } }
    end

    context "not logged in admin" do
      it_behaves_like("require_admin") { let(:action) { post :update, id: @video.id, video: {title: "New Title", category_ids: [@comedies.id.to_s], description: "A new movie instead" } } }
    end
    ...
    context "admin logged in" do
      ...
      context "valid arguments without file specification" do
        before do
          post :update, id: @video.id, video: {title: "New Title", category_ids: [@comedies.id.to_s], description: "A new movie instead" }
        end
      
        it "updates the video title and description" do
          expect(Video.all.size).to eq(1)
          expect(Video.first.title).to eq("New Title")
          expect(Video.first.description).to eq("A new movie instead")
        end

        it "updates the video's categories" do
          expect(Video.first.categories).to match_array([@comedies])
        end
        ...
        it "redirects to the show_video path for the edit video" do
          expect(response).to redirect_to video_path(Video.first)
        end
      end # valid args without supplied file
      ...
      context "valid arguments with supplied file" do
        before do
          image_large = Rack::Test::UploadedFile.new(Rails.root + "spec/support/attachments/monk_large.jpg")
          image_small = Rack::Test::UploadedFile.new(Rails.root + "spec/support/attachments/monk_small.jpg")
          post :update, id: @video.id, video: {title: "New Title", category_ids: [@comedies.id.to_s], description: "A new movie instead", large_cover: image_large, small_cover: image_small }
        end
