class VideosController < ApplicationController
  ...
  def show
    ...
    if (@video)
      ...
      if @video.reviews.find_by(user_id: current_user.id)
        @review = nil
      else
        @review = Review.new
      end
      ...
      if current_user.queued_videos.include? @video
        ...
        @video_queue_entry = VideoQueueEntry.new(
          position: current_user.queued_videos.size + 1, # position is at the end of the queue
